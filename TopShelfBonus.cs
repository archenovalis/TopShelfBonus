...
[assembly: MelonInfo(typeof(TopShelfBonus_IL2CPP.TopShelfBonus), "TopShelfBonus_Standard", "1.1.4", "Archie", "Adds 5% bonus for delivering items with quality above the customer's required standards.")]
  ...
  public static class NPCLoaderPatch
  {
    ...
    public static void LoadPostfix(NPCLoader __instance, DynamicSaveData saveData)
    {
      try
      {
        ...
        NPC npc = NPCManager.NPCRegistry._items.FirstOrDefault(x => x.ID == baseData.ID);
        if (npc == null) return;
